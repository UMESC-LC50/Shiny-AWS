...
s.1<-shapiro.test(dati.1$ab[location=="publication"])

s.2<-shapiro.test(dati.1$ab[location=="application"])

m.1<-leveneTest(ab~location, data=dati.1, center=mean)


results.lm <- lm(ab~location, data=dati.1)
aov.1<-aov(results.lm)
aov.2<-as.data.frame(summary(aov.1)[[1]])

tu.1<-TukeyHSD(aov.1, conf.level=0.95)
as.data.frame(tu.1$location)