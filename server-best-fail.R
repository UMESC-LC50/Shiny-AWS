...
output$example.1a<-renderDataTable(
...
#output$example.1b<-renderDataTable(
...
output$example.1b<-DT::renderDataTable(
  DT::datatable(example.1b(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
                                                              #order = list(list(2, 'asc'), list(1, 'asc')),
                                                              initComplete = JS(
                                                                "function(settings, json) {",
                                                                "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
...
output$example.1c<-renderDataTable(
...
output$example.1d<-renderDataTable(
  
  datatable(example.1d(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:')
  )),rownames = FALSE ))

getPage<-function() {
  return(includeHTML("model-stat.html"))
}
output$inc<-renderUI({getPage()})


#output$ex4 <- renderUI({
  #tags$iframe(src='model-stat.html',width="100%",frameBorder="0",height="1000px")
  
  #withMathJax(includeMarkdown("www/model-stat.md"))
