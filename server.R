...
#eval.config.sections(config.yaml)
Sys.setenv(R_TEST = "testit", "A+C" = 123, unset = NA)
...
shinyServer(function(input, output,session) {
  output$env.1<-renderText({
    as.character(Sys.getenv("R_TEST", unset = NA))
  })
    values <- reactiveValues()
  output$purpose <- renderUI({
    HTML(markdown::markdownToHTML(knit('www/purpose-11a.Rmd'),fragment.only = TRUE))
  })

  #output$foot.1 <- renderUI({
  #tags$iframe(src='footer.html',width="100%",overflow = "auto",allowtransparency="no",style="display:block;position:maximum;z-index:-900;")
  ...
  output$example<-renderTable({
    ...
    output$ex.1 <- renderUI({
      HTML(markdown::markdownToHTML(knitr::include_graphics('/srv/shiny-server/LC_calculator/example11.Rmd'),fragment.only = TRUE))
    })
    ...
    output$eventTimeRemaining <- renderText({
      ...
      paste("Calculation time remaining:", 
    ...
    #    else if (counter_createTime_Plot == 1) { 
          ...
          output$image2 <- renderPlot({ list(alt = "This is alternate text")
            par(family="serif") 
      ...
      #})
          ...
          my_output_data <- reactive({
            req(is.null(values$LC50.3)==FALSE)
            #View(head(values$LC50.3))
            #this loads the user selected data file into calculation for model selection 
            LC50.3<- stat2a()#values$LC50.3)
            LC50.1<-data1()
          ...
          output$compare.2<-renderDataTable(
            datatable(compare.2(),escape=FALSE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
                                                        'Combined results: ', htmltools::strong(
                                                        )),
                      options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),caption=input$species,
          ...
          output$stats2<-renderDataTable(
            #req(is.null(values$LC50.3)==FALSE),
            datatable(stats2b(),escape=FALSE,#server = TRUE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
                                                        'Models: ', htmltools::strong(
                                                        )),
                      options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),caption=input$species,
                                                                       #order = list(list(2, 'asc'), list(1, 'asc')),
                                                                       initComplete = JS(
          ...
          output$stats3<-renderDataTable(
            datatable(values$fish.14,escape=FALSE,#server = TRUE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
                                                        'Model statistics: ', htmltools::strong(
                                                        )),
                      options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),caption=input$species,
                                                                            #order = list(list(2, 'asc'), list(1, 'asc')),
                                                                            initComplete = JS(
          ...
          #output$stats3<-renderDataTable(
          #  datatable(req(result_val()),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
          #                                                                  #order = list(list(2, 'asc'), list(1, 'asc')),
          #                                                                  initComplete = JS(
          #                                                                    "function(settings, json) {",
          #                                                                    "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
          ...
          #)
            ...
            datatable(my_output_data(),escape=FALSE,#server = TRUE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
                                                        'Results: ', htmltools::strong(
                                                        )),
                      options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),caption = "Model Results",#container,
                                                                              #order = list(list(2, 'asc'), list(1, 'asc')),
                                                                              initComplete = JS(
                                                                                "function(settings, json) {",
                                                                                "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
          ...
          output$models<-renderDataTable(
            datatable(models.1(),escape=FALSE,#server = TRUE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
                                                                                                          'Models: ', htmltools::strong(
                                                                                                          )),
          ...
          output$data2<-renderDataTable(
            datatable(LC50.3(),escape=FALSE,#server = TRUE,
                      caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
