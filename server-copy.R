...
shinyServer(function(input, output,session) {
  ...
  # "values" is a matrix that will contain all global variables
  values <- reactiveValues()
  
  
  # test.1 is used in instruction on requirement for data file structure
  test.1<-read.csv("example.csv",header=TRUE,row.names=c(" ","  ","   "))
  ...
  #controls application to start calculation after file is loaded
  observe ({
    input$reset_input
    counter_createTime_Plot <<- 0
  })
  #the observeEvent is placed to control when the LC50 calculator to a user selected input 
  observeEvent(
    input$reset_input,{
      ...
      if (input$reset_input == 0)
        values<-NULL
      
      # run the function because the button is clicked once after loading a new file
      else if (counter_createTime_Plot == 1) {
        
        #stats2 is the variable where the LC50 model is selected
        #this is also where most of the global variables are calculated
        output$stats2 <- reactive({
          ...
          if(species.1%in%as.character(spec.1)==FALSE){
            ...
            values$time.1<-time.1
            values$end.1<-end.1
            values$how.1<-how.1
            values$species.1<-species.1
          ...
          values$fish.14<-as.data.frame(fb.6)
          fb.2
        })
  ...
  output$stats3<-DT::renderDataTable(
    datatable(values$fish.14,escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
                                                                    #order = list(list(2, 'asc'), list(1, 'asc')),
                                                                    initComplete = JS(
                                                                      "function(settings, json) {",
  ...
  output$image2 <- renderPlot({ 
    ...
    #this loads the user selected data file into calculation for model selection 
    LC50.3<-as.data.frame(values$LC50.3)
    ...
    how.1<-values$how.1#as.numeric(values$how.1)
    end.1<-values$end.1#as.numeric()
  ...
  output$data2<-DT::renderDataTable(
    datatable(LC50.a,escape=FALSE,caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center;font-weight: bold; color: #000000;',
  ...
  #this calculates the estimated concentration of LC25, LC50, and LC99
  my_output_data <- reactive({
    ...
    #this loads the first selected replicate to use in the calculation
    how.1<-as.numeric(values$how.1)
    end.1<-as.numeric(values$end.1)
  ...
  output$compare.2<-DT::renderDataTable(
    datatable(compare(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
                                                               #order = list(list(2, 'asc'), list(1, 'asc')),
                                                               initComplete = JS(
                                                                 "function(settings, json) {",
                                                                 "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
  ...
  output$markdown <- renderUI({
    HTML(markdown::markdownToHTML(knit('www/purpose.Rmd'),fragment.only = TRUE))#, quiet = FALSE,rmarkdown=TRUE))
  })
  ...
  coef.1<-reactive({
    ...
    if(LW.1=="yes"){
      ...
      rep.5<-as.numeric(unique(LC50.3$replicate))
      estimate.1<-data.frame(estimate=c(rep(0,4)),error=c(rep(0,4)),tvalue=c(rep(0,4)),pvalue=c(rep(0,4)))
      error.1<-data.frame(estimate=c(rep(0,4)),error=c(rep(0,4)),tvalue=c(rep(0,4)),pvalue=c(rep(0,4)))
      tvalue.1<-data.frame(estimate=c(rep(0,4)),error=c(rep(0,4)),tvalue=c(rep(0,4)),pvalue=c(rep(0,4)))
      pvalue.1<-data.frame(estimate=c(rep(0,4)),error=c(rep(0,4)),tvalue=c(rep(0,4)),pvalue=c(rep(0,4)))
      ...
      for (i.1 in 1:s.4){
        ...
        fish.drm.2a<-drm(effect/total~conc,data=LC50.3,replicate,fct=model.2a(),type="continuous",separate=TRUE)
        
        sum.1<-signif(summary(fish.drm.2a)$coefficients,4)
        variable<-row.names(sum.1)
        names(sum.1)<-c("estimate.1","variable","error.1","tvalue.2","pvalue.2")
        sum.2<-cbind.data.frame(variable,sum.1)
        estimate<-sum.2[i.1]<-sum.2[,1]
        error.1<-sum.2[i.1]<-sum.2[,1]
        tvalue<-sum.2[i.1]<-sum.2[,1]
        pvalue<-sum.2[i.1]<-sum.2[,1]
      }
      
      sum.3<-as.data.frame(sum.2)
      replicate<-c(how.1:end.1)
      sum.4<-cbind.data.frame(replicate,sum.3)
      
      sum.4
      
    }
  })
  output$coef.1<-DT::renderDataTable(
    datatable(coef.1(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
                                                              #order = list(list(2, 'asc'), list(1, 'asc')),
                                                              initComplete = JS(
                                                                "function(settings, json) {",
                                                                "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
...
coef.1<-reactive({
  ...
  if (input$LW=="yes"){
    ...
    tt.3<-data.frame(c("This model","only estimates","one replicate.","No pooling of data."))
    names(tt.3)<-"Model information"
      ...
      for (i.1 in 1:s.4){
        ...
        model.2a<-assign(function.name,get(function.name))
        #loads the dose reponse model global variable into calculation
        fish.drm<-drm(effect/total~conc,data=LC50.4,fct=model.2a(),type="continuous",separate=FALSE)
        tt.1a<-as.data.frame(summary(fish.drm)$coefficients)
        
        
        tt.1<-cbind.data.frame(row.names(tt.1a),tt.1a[,1],tt.1a[,2],tt.1a[,3],tt.1a[,4])
        names(tt.1)<-c("variable","estimate","std error","t-value","p-value")
        tt.1$replicate<-i.1
        tt.1$model<-rep(function.name,nrow(tt.1))
      tt.2[[i.1]]<-tt.1
      }
      tt.3x<-do.call(rbind.data.frame,tt.2)
      tt.3b<-tt.3x[,c(6,7,1,2,3,4,5)]
      tt.3a<-signif(tt.3b[,4:7],4)
      tt.3<-cbind.data.frame(tt.3b[,1:3],tt.3a)
...
models.1<-reactive({
  uu<-as.data.frame(t(read.csv("models.csv",header=TRUE)))
  uu.1<-uu[2:30,1]
  uu.2<-uu[2:30,2]
  uu.3<-cbind.data.frame(uu.1,uu.2,row.names = NULL)
  names(uu.3)<-c("abbreviations","model description")
  uu.3
})

output$models<-DT::renderDataTable(
  datatable(models.1(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:'),
                                                            #order = list(list(2, 'asc'), list(1, 'asc')),
                                                            initComplete = JS(
                                                              "function(settings, json) {",
                                                              "$(this.api().table().header()).css({'background-color': '#337ab7', 'color': '#fff'});",
...
output$example.1a<-DT::renderDataTable(
  datatable(example.1a(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:')
  )),rownames = FALSE ))
...
output$example.1b<-DT::renderDataTable(
  datatable(example.1b(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:')
  )),rownames = FALSE ))

output$purpose <- renderUI({
  HTML(markdown::markdownToHTML(knit('www/purpose-11.md'),fragment.only=TRUE))
})
...
output$example.1c<-DT::renderDataTable(
  
  datatable(example.1c(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:')
  )),rownames = FALSE ))
...
output$example.1d<-DT::renderDataTable(
  
  datatable(example.1d(),escape=FALSE,options(DT.options = list(pageLength = 25, language = list(search = 'Filter:')
  )),rownames = FALSE ))
